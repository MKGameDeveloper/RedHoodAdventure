...
void Graphics::Resize(const UINT & width, const UINT & height)
{
	...
	DeleteSurface();
	{
		...
		(
			...
			width,					// 백 버퍼의 너비
			height,					// 백 버퍼의 높이
			DXGI_FORMAT_UNKNOWN,	// 백 퍼버의 포맷 (기존과 동일하게 유지)
...
void Graphics::InitViewport(int num)
{
	// OM단계에서 사용할 뷰포트를 설정
	deviceContext->RSSetViewports(num, &viewport);
}

// 랜더링을 시작하기 전 필요한 초기화 작업을 수행
void Graphics::Begin()
{
	// RTV 설정
...
void Graphics::CreateSwapchain()
{
	...
	// 가상 VRAM이 큰 어댑터를 선택하여 selectedAdapterIndex에 할당
	UINT maxVRam = 0;
	...
	(
		...
		D3D11_CREATE_DEVICE_BGRA_SUPPORT,												// 렌더링 시 필요한 플래그 설정
		featureLevel.data(),							// 사용할 Direct3D 기능 레벨
		featureLevel.size(),							// 사용할 Directr3D 기능 레벨의 개수
