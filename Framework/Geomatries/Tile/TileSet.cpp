    ...
    ImGui::Begin("TileSet");
    {
        ...
        for (uint y = 0; y < tileYCount; y++)
        {
            for (uint x = 0; x < tileXCount; x++)
            {
                ...
                ImGui::PushID(count);
                if (ImGui::ImageButton(tileSRV, ImVec2(15, 15), ImVec2(startX, startY), ImVec2(startX + texelTileSize.x, startY + texelTileSize.y)))
...
TileSet::TileSet()
{
    Texture2D* tex = new Texture2D(TexturePath + L"TileMap(34_63).png");
    tileSRV = tex->GetSRV();
    SAFE_DELETE(tex);
